...
\caption{\csentence{Corrected dffect plot and Bland-Altman plots.} The same dataset as in Figure \ref{bv} with the IQLR, Zero replacement or LVHA corrections applied. All three corrections place the ribosomal protein functions and glycolytic functions closer to zero, as desired, but the asymmetry can only be adequately addressed by the LVHA defined correction with geometric mean of the low variance high relative abundance functions as the denominator. BA: Bland-Altman plot }
\label{Fig:ribo}
\end{figure}
